/**
 * Import GSIS member-fillout PDFs from a curated list into:
 *   1) public/forms/NoFormEditor/  (renamed with "GSIS - " prefix)
 *   2) qfph.db forms table         (idempotent upsert by slug)
 *
 * Source list: /tmp/gsis_forms_to_ingest.json
 *   [{file, title, url, pages, acro}, ...]
 *
 * Usage: npx tsx scripts/import_gsis_no_editor.ts
 */
import fs from 'node:fs';
import path from 'node:path';
import Database from 'better-sqlite3';

const SRC_DIR = '/home/skouzen/projects/quickformsph/Forms-Research/GSIS/pdf';
const DEST_DIR = '/home/skouzen/projects/quickformsph-dev/public/forms/NoFormEditor';
const TASK_JSON = '/tmp/gsis_forms_to_ingest.json';
const DB_PATH = '/tmp/qfph/qfph.db';

interface Task {
  file: string;     // basename in SRC_DIR
  title: string;    // display name from gsis_forms.json (or derived)
...
function deriveCode(file: string): string {
  // Strip date prefixes and "Forms-" / "FORMS-" / "Form -" markers, then keep core
  let base = file.replace(/\.[Pp][Dd][Ff]$/, '');
  base = base.replace(/^\d{8}[-_ ]/, '');
  base = base.replace(/^Form[-_ ]No[-_ ]\d+[-_ ]/i, '');
  base = base.replace(/^(FORMS?|Forms?|Form)[-_ ]/g, '');
  // Collapse to alnum-dash
  return base.replace(/[^A-Za-z0-9]+/g, '-').replace(/^-|-$/g, '');
}

function slugify(s: string): string {
  return ('gsis-' + s.toLowerCase())
    .replace(/[^a-z0-9-]+/g, '-')
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '');
}
...
function destFilename(srcFile: string): string {
  // Always prefix "GSIS - " for consistent NoFormEditor naming
  const cleaned = srcFile.replace(/^GSIS[- _]+/i, '');
  return `GSIS - ${cleaned}`;
}
...
function main() {
  ...
  console.log(`[*] ${ready.length} GSIS PDFs to import`);
  ...
  const insertStmt = db.prepare(`
    ...
    VALUES
      (@slug, @form_code, @form_name, 'GSIS', @pdf_path, @source_url,
       0, 0, 0, 0,
  ...
  for (const r of ready) {
    ...
    const formName = `GSIS — ${title}`;
    const pdfPath = `NoFormEditor/${path.basename(r.dst)}`;
    const desc = `Official GSIS form: ${title}.`;
    const params = {
      slug: r.slug, form_code: r.code, form_name: formName, agency: 'GSIS',
  ...
  const total = db.prepare("SELECT COUNT(*) AS n FROM forms WHERE agency='GSIS' AND deleted_at IS NULL").get() as { n: number };
  console.log(`[*] Total GSIS rows in DB now: ${total.n}`);
  db.close();
}
