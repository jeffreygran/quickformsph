/**
 * Import DFA member-fillout PDFs from Forms-Research/DFA/pdf/ into:
 *   1) public/forms/NoFormEditor/  (filename already has "DFA - " prefix)
 *   2) qfph.db forms table         (idempotent upsert by slug)
 *
 * Usage: npx tsx scripts/import_dfa_no_editor.ts
 */
import fs from 'node:fs';
import path from 'node:path';
import Database from 'better-sqlite3';

const SRC_DIR = '/home/skouzen/projects/quickformsph/Forms-Research/DFA/pdf';
const DEST_DIR = '/home/skouzen/projects/quickformsph-dev/public/forms/NoFormEditor';
const CATALOG_JSON = '/home/skouzen/projects/quickformsph/Forms-Research/DFA/dfa_forms.json';
...
function main() {
  ...
  console.log(`[*] ${tasks.length} DFA PDF forms to import`);
  ...
  const insertStmt = db.prepare(`
    ...
    VALUES
      (@slug, @form_code, @form_name, 'DFA', @pdf_path, @source_url,
       0, 0, 0, 0,
  ...
  for (const t of tasks) {
    ...
    const formName = `DFA — ${e.name.trim()}`;
    const pdfPath = `NoFormEditor/${path.basename(t.destFile)}`;
    const description = (e.description && e.description.trim()) || `Official DFA ${e.name.trim()}.`;
    const params = {
      slug, form_code: e.code, form_name: formName, agency: 'DFA',
  ...
  const total = db.prepare("SELECT COUNT(*) AS n FROM forms WHERE agency='DFA' AND deleted_at IS NULL").get() as { n: number };
  console.log(`[*] Total DFA rows in DB now: ${total.n}`);
  db.close();
}
